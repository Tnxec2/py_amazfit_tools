...
class TodayElement(ContainerElement):
    ...
    def createChildForParameter(self, parameter):
        ...
        if parameterId == 1:
            from watchFaceParser.models.elements.weather.separateTemperature import SeparateTemperatureElement
            self._separate_temperature = SeparateTemperatureElement(parameter = parameter, parent = self, name = 'SeparateTemperature')
            return self._separate_temperature
        elif parameterId == 2:
            from watchFaceParser.models.elements.weather.oneLineTemperature import OneLineElement
            self._oneline = OneLineElement(parameter = parameter, parent = self, name = 'OneLine')
            return self._oneline
        else:
            return super(TodayElement, self).createChildForParameter(parameter)
